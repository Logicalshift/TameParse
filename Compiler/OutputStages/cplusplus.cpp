...
std::string get_identifier(const std::wstring& name) {
	...
	for (wstring::const_iterator wideChar = stripped.begin(); wideChar != stripped.end(); wideChar++) {
		...
		if ((wideChar >= 'A' && wideChar <= 'Z') || (wideChar >= 'a' && wideChar <= 'z') || (res.size() > 0 && wideChar >= '0' && wideChar <= '9')) {
			res << (char) wideChar;
			continue;
		}
...
virtual void write_header(const std::wstring& filename, std::ostream* target) {
	(*target) << "///\n";
	(*target) << "/// " << get_identifier(filename) << "\n";
	(*target) << "/// Parser file generated by TameParse at " << strftime("%a %Y/%m/%d %H:%M:%S") << "\n";
	(*target) << "///\n\n"
}
